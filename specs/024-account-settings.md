...
| `?kilpailu=` present but invalid | Existing behaviour is unchanged: the region's **hardcoded** fallback and the existing notice. The preference does not silently rescue a bad URL, because the notice would then name a competition the reader never asked for. |
