...
| What may be a baseline season | A competition the registry **knows** to be a league | Third review round, raised as raw competition names reaching Finnish UI copy. Both name lookups fall back to the code itself, and both format tests answer "league" for an unknown code — `getCompetitionFormat` says so in its own comment. So a stored season whose competition had left the registry would join the baseline, add its matches to every pooled rate, and put `ZZZ` in a Finnish sentence. Testing for a known league fixes the rate and the sentence at once. |
| A **pass-through** season, which ranks nobody | Kept, with a null position | Its matches are league matches; only its published table disagrees with ours. Dropping it would quietly shrink the baseline of every rate to protect one row. |
| A season played entirely in **knockout** groups | Left out | It has no *league* match at all: `teamLeagueMatches` counts table groups only, which is the same rule `Vire`, `Maalit` and the standings table apply. Counting them would put matches in the baseline that the selected season's own measures exclude. An earlier draft of this row said "pass-through or knockout … its results still count", which was wrong about knockout and is what a reviewer read as a requirement the code broke. |
...
- **Twenty-six mutations**, all caught, listed below.

### The mutations

Twenty-six, in six groups, each of which failed at least one test.
...
**The baseline's membership (2), added after the third.** Each provider
admitting a competition the registry does not carry, by testing the format
instead of testing for a known league.
