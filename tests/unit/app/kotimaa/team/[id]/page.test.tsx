...
vi.mock("@/lib/taso-standings-service", async (importOriginal) => {
  ...
  return { ...actual, getTeamMatches: getTeamMatchesMock };
});
...
async function renderTeam(
  ...
  const { default: KotimaaTeamPage } = await import("@/app/kotimaa/team/[id]/page");
  render(
    await KotimaaTeamPage({
...
describe("Kotimaa team page", () => {
  ...
  it("defaults searchParams handling when none is provided", async () => {
    const { default: KotimaaTeamPage } = await import("@/app/kotimaa/team/[id]/page");
    render(await KotimaaTeamPage({ params: Promise.resolve({ id: "1" }) }));

    expect(screen.getByRole("heading", { name: "HJK – Veikkausliiga 2026" })).toBeInTheDocument();
  });

  it("sets the browser tab title to the team's name", async () => {
    const { generateMetadata } = await import("@/app/kotimaa/team/[id]/page");
