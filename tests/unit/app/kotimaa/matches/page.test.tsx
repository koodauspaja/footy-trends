...
async function renderMatches(searchParams: Record<string, string | string[] | undefined> = {}) {
  const { default: KotimaaMatchesPage } = await import("@/app/kotimaa/matches/page");
  render(await KotimaaMatchesPage({ searchParams: Promise.resolve(searchParams) }));
}

describe("Kotimaa matches page", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("defaults searchParams handling when none is provided", async () => {
    const { default: KotimaaMatchesPage } = await import("@/app/kotimaa/matches/page");
    render(await KotimaaMatchesPage({}));

    expect(screen.getByRole("heading", { name: "Veikkausliiga 2026" })).toBeInTheDocument();
  });

  it("defaults tab title handling when searchParams is not provided", async () => {
    const { generateMetadata } = await import("@/app/kotimaa/matches/page");
